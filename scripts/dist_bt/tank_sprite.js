...
export class TankSprite extends Moveable {
     ...
     * @param {number} turnRate step the turn behavior with a rate factor.
     */
     ...
     * 
     * @returns a BT status for to advance the behavior tree traversal.
     */
    targetForDestruction( turnRate = 0.75 ) {
        ...
        // Compute the DELTA between ideal and current orientaion...
        const idealRotation = normal.getOrientation();
        const currentRotation = this.getOrientation();
        let delta = idealRotation - currentRotation;
        // Adjust your orientation by some fraction of the delta
        // UNTIL you have locked on...
        const tolerance = 1.1;
        while( Math.abs( delta ) > tolerance ) {
            console.log("targeting...");
            const newRotation = currentRotation + turnRate * delta;
            this.setOrientation( newRotation ) ; 
            return btConstants.IN_PROGRESS;
        }
